(function() {
    ...
    var default_circle_style = {
        ...
        circleContainerStyle: 'text-align:center;padding:30px;'
    };
    ...
    function makeProgressCircle($scope, $compile) {
        return angular.element($compile('<div style="{{circleContainerStyle}}"><div round-progress  max="max"  responsive="true"  current="progress"  color="{{color}}" bgcolor="{{bgcolor}}"  radius="{{radius}}"  stroke="{{stroke}}"  rounded="rounded" clockwise="clockwise" iterations="{{iterations}}"  animation="{{animation}}"></div></div>')($scope));
    };
    ...
    var uiOnStart = function(scope, element, $compile, uiData) {
        ...
        if (scope.srcIs == 'background') {
            elm.css({'background':scope.backgroundLoadingStyle,'background-size':'15%'});
